import PaymentIcons from '@/Components/Home/PaymentIcons';

const SOCIALS = [
    {
        label: 'Facebook',
        href: 'https://www.facebook.com/people/Captivating-Cabinets/61577724460661/',
        path: 'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z',
    },
    {
        label: 'Instagram',
        href: 'https://www.instagram.com/captivatingcabinets',
        path: 'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z',
    },
];

const COLUMNS = [
    {
        title: 'Product',
        links: [
            { label: 'Products', href: '#' },
            { label: 'Brands', href: '/cabinet-brands' },
            { label: 'Contractor Program', href: '/contractor-program' },
            { label: 'Financing', href: '/financing' },
            { label: 'Get Free Design', href: '/design-service' },
        ],
    },
    {
        title: 'Brands',
        links: [
            { label: 'Fabuwood', href: '/brands/fabuwood' },
            { label: 'J&K', href: '/brands/jk' },
            { label: 'Modernform', href: '/brands/modernform' },
            { label: 'Diamond', href: '/brands/diamond' },
            { label: 'Decora', href: '/brands/decora' },
            { label: 'Mantra', href: '/brands/mantra' },
            { label: 'Tribeca', href: '/brands/tribeca' },
            { label: 'KCD', href: '/brands/kcd' },
            { label: 'USCD', href: '/brands/uscd' },
        ],
    },
    {
        title: 'Resources',
        links: [
            { label: '3D Design Service', href: '/design-service' },
            { label: 'Specbooks', href: '/resources/specbooks' },
            { label: 'Shipping FAQ', href: '#' },
            { label: 'Kitchen Glossary', href: '#' },
            { label: 'Blog', href: '/blogs' },
            { label: 'Testimonials', href: '/testimonials' },
            { label: 'About', href: '#' },
            { label: 'Contact', href: '/contact' },
        ],
    },
    {
        title: 'Legal',
        links: [
            { label: 'Privacy', href: '#' },
            { label: 'Terms', href: '#' },
            { label: 'Accessibility', href: '#' },
            { label: 'Cookies', href: '#' },
            { label: 'Shipping Policy', href: '#' },
        ],
    },
];

export default function Footer() {
    const year = 2026;
    return (
        <footer className="bg-[#F3F4F6] text-[#374151] border-t border-[#374151]/10">
            <div className="max-w-container mx-auto px-6 lg:px-8 py-14 lg:py-16">
                <div className="grid gap-10 lg:gap-8 sm:grid-cols-2 lg:grid-cols-5">
                    {/* Brand */}
                    <div className="lg:col-span-1 sm:col-span-2">
                        <a href="/" className="inline-flex" aria-label="Captivating Cabinets home">
                            <img
                                src="/images/captivating-cabinets.svg"
                                alt="Captivating Cabinets"
                                className="h-10 w-auto"
                            />
                        </a>
                        <p className="mt-4 text-[#374151]/60 text-sm leading-relaxed max-w-xs">
                            Premium cabinetry with complimentary 3D design, contractor pricing, and nationwide delivery.
                        </p>

                        {/* Social */}
                        <div className="mt-5 flex items-center gap-3">
                            {SOCIALS.map(({ label, href, path }) => (
                                <a
                                    key={label}
                                    href={href}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    aria-label={label}
                                    className="w-9 h-9 flex items-center justify-center rounded-full bg-[#374151]/8 text-[#374151] hover:bg-amber-400 hover:text-gray-900 transition-colors"
                                >
                                    <svg viewBox="0 0 24 24" className="w-[18px] h-[18px]" fill="currentColor" aria-hidden="true">
                                        <path d={path} />
                                    </svg>
                                </a>
                            ))}
                        </div>
                    </div>

                    {/* Link columns */}
                    {COLUMNS.map(({ title, links }) => (
                        <nav key={title} aria-label={title}>
                            <h3 className="text-[#374151] text-sm font-semibold tracking-wide uppercase">{title}</h3>
                            <ul className="mt-4 space-y-2.5">
                                {links.map(({ label, href }) => (
                                    <li key={label}>
                                        <a
                                            href={href}
                                            className="text-[#374151]/65 hover:text-amber-600 text-sm transition-colors"
                                        >
                                            {label}
                                        </a>
                                    </li>
                                ))}
                            </ul>
                        </nav>
                    ))}
                </div>
            </div>

            {/* Bottom bar */}
            <div className="border-t border-[#374151]/10">
                <div className="max-w-container mx-auto px-6 lg:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p className="text-[#374151]/50 text-xs order-2 sm:order-1">
                        © {year} Captivating Cabinets. All rights reserved.
                    </p>
                    <div className="order-1 sm:order-2">
                        <PaymentIcons />
                    </div>
                </div>
            </div>
        </footer>
    );
}
